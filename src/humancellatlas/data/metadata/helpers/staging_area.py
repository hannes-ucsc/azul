...
class StagingArea:
    ...
    def get_bundle(self, subgraph_id: str) -> Bundle:
        ...
        return Bundle(subgraph_id, links_file.version, manifest, metadata)
...
class CannedStagingAreaFactory:
    ...
    def _get_descriptor_files(self,
                              staging_area: Path
                              ) -> dict[str, DescriptorFile]:
        files = {}
        path = staging_area / 'descriptors'
        for sub_dir in path.iterdir():
            assert sub_dir.is_dir()
    ...
    def _add_file(self, files: dict[str, JSON_FILE], file: JSON_FILE) -> None:
        """
        Add `file` to `files`. If a file with the same ID already exists, the
        one with the most recent version will be kept.
        """
        try:
