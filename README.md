...
The steps below are examplary for Python 3.11.7. Replace `3.11.7` with the 
...
Set `PYENV_VERSION` to `3.11.7` in `environment.local.py` at the project root.
...
```
(.venv) ~/workspace/hca/azul$ docker run -it -v ${project_root}/:/root/azul python:3.11.7-bullseye bash
